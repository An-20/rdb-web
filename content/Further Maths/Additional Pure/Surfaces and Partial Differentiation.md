...
$$
#### Stationary points
A **stationary point** is where $\text{f}_{x}= 0$ and $\text{f}_{y} = 0$. These are classified as minima, maxima, or saddle points, depending on the determinant of the **Hessian matrix**:
$$
|\boldsymbol{H}| = 
\left|\begin{matrix} \text{f}_{xx} & \text{f}_{xy} \\ \text{f}_{yx} & \text{f}_{yy} \end{matrix}\right| 
= \text{f}_{xx}\text{f}_{yy} - \text{f}_{xy}\text{f}_{yx}
$$
- If $|\boldsymbol{H}| \gt 0$ and $\text{f}_{xx} > 0$, there is a **minimum**.
- If $|\boldsymbol{H}| \gt 0$ and $\text{f}_{xx} < 0$, there is a **maximum**.
- If $|\boldsymbol{H}| \lt 0$ there is a **saddle point**.
- If $|\boldsymbol{H}| = 0$ the nature of the stationary point cannot be determined by this test.

#### Tangent planes
The **tangent plane** to a surface $z = \text{f}(x, y)$ at the point $(a,\,b,\,\text{f}(a, b))$ is given by:
$$
