...
- Start with the **reduced equation** $u_{n+1} = ku_n$ (only the homogeneous part).
...
- Start with the **reduced equation** $u_{n+2} = au_{n+1} + bu_n$ (only the homogeneous part).
- Replace $u_n$ with $r^n$ ($r \not= 0$), giving the **auxiliary equation**.
- Solve for $r$, giving two possible roots $r_1$ and $r_2$.
The **complementary function** depends on the nature of the roots $r_1$ and $r_2$:
- For **distinct real or complex roots** $r_1$ and $r_2$, $c(n) = {Ar_1}^n + {Br_2}^n$.
