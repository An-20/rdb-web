...
**Induction** is where a changing magnetic flux in a circuit causes an electromotive force to be induced in the circuit. In real circuits, coils with many turns are used, which is why flux linkage is used. **Flux linkage**, $N\Phi$ (measured in weber turns), is equal to the flux multiplied by the number of turns in the coil.
...
In a **magnetic circuit**, the flux lines form a closed loop. Similar to conductance in electrical circuits, magnetic circuits have **permeance**; higher permeance means more flux. Thus, magnetic circuits are often made from ferromagnetic materials with a **high magnetic permeability**, for example iron. (_IS_) The magnetic equivalent of p.d. is **current-turns** $NI$ (equal to the current in a coil multiplied by the number of turns), while the magnetic equivalent of electric current is flux $\Phi$. As in a electrical circuit, higher current-turns will cause a higher flux.
