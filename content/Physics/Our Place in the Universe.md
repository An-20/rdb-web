...
_NIS_: Parallax is never explicitly mentioned in the specification.
...
% earth positions
\filldraw[blue] (EarthTop) circle (0.11);
\filldraw[blue] (EarthBot) circle (0.11);
...
% apparent positions
\filldraw[red!60] (FarFromTop) circle (0.13);
\filldraw[red!60] (FarFromBot) circle (0.13);
...
_NIS_: Standard candles are never explicitly mentioned in the specification.
...
The following formulae are all _given_ in the formula booklet:
...
_IS_: Time dilation is explicitly mentioned in the specification. The derivation, however, is very likely not required.
...
For an observer watching the clock travel past at speed $v$, the Pythagorean theorem can be used (where $t$ is the time measured by the observer, and $\tau$ is the time measured by the clock):
...
So to the observer, the time taken per tick was $t = \tau\gamma$. As $\gamma \ge 1$, the observer measures a longer time per tick than the clock experiences, meaning the clock appears to tick slower.

In general, time passes more slowly for objects moving at high speeds, compared to stationary observers. The **time dilation** is given by $t = \gamma\tau$, which is _not on_ the formula booklet.

#### Length contraction

_NIS_: Length contraction is not explicitly mentioned on the specification.
...
- The stationary observer measures the particle to take time $t$ to travel distance $L_0$ between two points, giving $v = \frac{L_0}{t}$
- A clock moving with the particle measures a time $\tau = \frac{t}{\gamma}$ for those points a distance $L$ to fly past, giving $v = \frac{L}{\tau} = \frac{L\gamma}{t}$
- The relative velocities in both frames must be identical, so $v = \frac{L\gamma}{t} = \frac{L_0}{t}$.
- Rearranging gives $L = \frac{L_0}{\gamma}$.

So to the particle, the length was $L = \frac{L_0}{\gamma}$, while from the stationary observer, the length was $L_0$. As $\gamma \ge 1$, the particle travels a smaller distance in its own frame of reference compared to a stationary observer.

Length contraction is symmetric in the sense that both observers measure shorter lengths. If the particle was a rod moving at relativistic speeds, the stationary observer would measure the rod to be shorter, and the rod would measure itself to have travelled a shorter distance.
