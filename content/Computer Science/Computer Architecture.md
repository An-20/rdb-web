...
## Secondary Storage
#### Hard disk
A hard disk drive contains multiple rapidly spinning platters with a magnetic coating. Each platter consists of concentric tracks and radially segmented sectors. Each platter has a read-write head, which moves radially to access different tracks. To read and write data, the read-write head moves to the desired sector. To write, the head uses an electromagnet to change the magnetic polarity of specific areas in the sector, with one polarity representing a binary 1 and the other representing a 0. To read, an electromagnetic records the changing polarity and converts this to a binary signal. 
